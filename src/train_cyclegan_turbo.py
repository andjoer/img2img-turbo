...
def main(args):
    ...
    noise_scheduler_1step = make_1step_sched(model=model,device=accelerator.device)
    text_encoder = CLIPTextModel.from_pretrained("stabilityai/sd-turbo", subfolder="text_encoder").to(accelerator.device)
    ...
    vae_a2b, vae_lora_target_modules = initialize_vae(args.lora_rank_vae, return_lora_module_names=True,device=accelerator.device)
