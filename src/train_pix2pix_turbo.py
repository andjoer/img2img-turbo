...
def main(args):
    ...
    net_pix2pix = Pix2Pix_Turbo(model_name = args.pretrained_model_name_or_path,pretrained_path=args.pretrained_path, is_sdxl=args.is_sdxl,lora_rank_unet=args.lora_rank_unet, lora_rank_vae=args.lora_rank_vae,
        device=accelerator.device,tokenizers=tokenizers,text_encoders=text_encoders,freeze_unet=args.freeze_unet,freeze_vae=args.freeze_vae,no_skips=args.no_skips,)
    ...
    if args.gan_disc_type == "vagan_clip":
        net_disc = cv_discriminator.Discriminator(cv_type='clip', in_ch1=disc_in_ch, in_ch2=disc_in_ch2, loss_type=args.gan_loss_type, device=accelerator.device)
    else:
    ...
    layers_to_opt = net_pix2pix.get_traininable_params()
    ...
    for epoch in range(0, args.num_training_epochs):
        for step, batch in enumerate(dl_train):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                if "mps" in str(accelerator.device):    # size needs to be dividable by 224
                    x_src_resized = F.interpolate(x_src, size=(resize_to, resize_to), mode='bilinear')
                ...
                if args.double_disc:
                    lossG = net_disc(x_tgt_pred_resized, images2=x_src_resized, for_G=True).mean() * args.lambda_gan
                else:
                ...
                # real image
                if args.double_disc:
                    lossD_real = net_disc(x_tgt_resized.detach(), images2=x_src_resized, for_real=True).mean() * args.lambda_gan
                else:
                ...
                # fake image
                if args.double_disc:
                    lossD_fake = net_disc(x_tgt_pred_resized.detach(), images2=x_src_resized, for_real=False).mean() * args.lambda_gan
                else:
    ...
    if accelerator.is_main_process:      
        outf = os.path.join(args.output_dir, "checkpoints", f"final_model_{global_step}.pkl")          
